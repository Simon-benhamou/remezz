...
export const api = {
  ...
  getSelectorSnapshot: async (options?: { force?: boolean }) =>
  ...
  backtest: {
    run: async (params: { startDate: string; endDate: string; initialCapital: number; symbols: string[]; leverage: number }) =>
      (await client.post('/api/backtest/run', params, { timeout: 15 * 60 * 1000 })).data,
    getPresets: async () =>
  ...
  polymarket: {
    ...
    saveCredentials: async (creds: { privateKey: string; apiKey: string; apiSecret: string; apiPassphrase: string }) =>
      (await client.put('/api/polymarket/credentials', creds)).data,
