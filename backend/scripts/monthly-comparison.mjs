 ...
 * Run with: node scripts/monthly-comparison.mjs
 */
import { runBacktest } from '../dist/src/services/backtestService.js';
import { preloadMarkets } from '../dist/src/exchange/ccxtClient.js';
...
function getMonths() {
  ...
  for (let m = 0; m < 12; m++) {
    months.push({
      label: `${names[m]} 2025`,
      start: new Date(Date.UTC(2025, m, 1)),
      end: new Date(Date.UTC(2025, m + 1, 0, 23, 59, 59)),
    });
  }
  months.push({
    label: 'Jan 2026',
...
async function main() {
  ...
  const loaded = await preloadMarkets();
  if (!loaded) {
    console.error('❌ Failed to preload markets — cannot run backtest');
    process.exitCode = 1;
    return;
  }
  ...
  for (const m of months) {
    ...
    try {
      ...
      console.log(`✅ ${result.summary.totalTrades} trades, PnL ${result.summary.totalPnlPct.toFixed(2)}%`);
    } catch (err) {
  ...
  const sorted2025 = months2025.map(m => m.summary.totalPnlPct).sort((a, b) => a - b);
  const medianPnl2025 = sorted2025[Math.floor(sorted2025.length / 2)];
