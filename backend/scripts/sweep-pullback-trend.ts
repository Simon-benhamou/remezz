/**
 * Pullback Trend Parameter Sweep
 * Tests 10 configurations across 2024 and 2025 with BTC, ETH, SOL, XRP
 *
 * Usage: npx tsx scripts/sweep-pullback-trend.ts 2>&1
 */
...
import { PullbackTrendStrategy } from '../src/strategies/pullbackTrend/strategy.js';
...
// ============================================================================
const DEFAULTS = { ...PULLBACK_CONFIG };

function resetConfig(): void {
  const cfg = PULLBACK_CONFIG as any;
...
const CONFIGS: SweepConfig[] = [
  {
    name: 'A',
    label: 'Baseline SMA50 BB(20,2) SL=2%',
    overrides: {},
  },
  {
    name: 'B',
  ...
  {
    name: 'C',
    label: 'Wider trail 1.5%/2.5%',
    overrides: { TRAILING_ACTIVATION_PCT: 1.5, TRAILING_DISTANCE_PCT: 2.5 },
  },
  {
    name: 'D',
    label: 'No BTC filter',
    overrides: { TREND_REQUIRE_BTC_ALIGN: false },
  },
  {
    name: 'E',
    label: 'Faster trend SMA20',
    overrides: { TREND_SMA_PERIOD: 20 },
  },
  {
    name: 'F',
    label: 'Slower trend SMA100',
    overrides: { TREND_SMA_PERIOD: 100 },
  },
  {
    name: 'G',
    label: 'Relaxed RSI 40/60',
    overrides: { RSI_OVERSOLD: 40, RSI_OVERBOUGHT: 60 },
  },
  {
    name: 'H',
    label: 'No volume filter',
    overrides: { VOLUME_MIN: 0 },
  },
  {
    name: 'I',
    label: 'Combined best SL1.5 trail wide RSI40/60',
    overrides: {
      STOP_LOSS_PCT: 1.5,
  ...
  {
    name: 'J',
    label: 'Aggressive BB(20,1.5) ADX10',
    overrides: { BB_STD: 1.5, ADX_MIN: 10 },
  },
];
...
async function main() {
  ...
  console.log('PULLBACK TREND PARAMETER SWEEP');
  console.log('='.repeat(80));
  ...
  console.log(`Leverage: 3x (strategy default)`);
  console.log('');
    ...
    for (const year of YEARS) {
      ...
      const input: BacktestComputationInput = {
        params: {
          ...
          leverage: stratConfig.leverage || 3,
          strategy,
  ...
  const header = `${'Cfg'.padEnd(4)} ${'Label'.padEnd(38)} ${'Year'.padEnd(5)} ${'Trades'.padStart(7)} ${'WR%'.padStart(6)} ${'PnL $'.padStart(10)} ${'Sharpe'.padStart(7)} ${'DD%'.padStart(6)} ${'PF'.padStart(6)} ${'AvgWin'.padStart(8)} ${'AvgLoss'.padStart(8)} ${'W/L'.padStart(5)}`;
  console.log(header);
  console.log('-'.repeat(120));

  for (const r of allResults) {
    const line = `${r.config.padEnd(4)} ${r.label.padEnd(38)} ${String(r.year).padEnd(5)} ${String(r.trades).padStart(7)} ${r.winRate.toFixed(1).padStart(6)} ${('$' + r.pnl.toFixed(0)).padStart(10)} ${r.sharpe.toFixed(2).padStart(7)} ${r.maxDD.toFixed(1).padStart(6)} ${r.profitFactor.toFixed(2).padStart(6)} ${('$' + r.avgWin.toFixed(1)).padStart(8)} ${('$' + r.avgLoss.toFixed(1)).padStart(8)} ${r.winLossRatio.toFixed(2).padStart(5)}`;
    console.log(line);
  }
  ...
  const rHeader = `${'Rank'.padEnd(5)} ${'Cfg'.padEnd(4)} ${'Label'.padEnd(38)} ${'Sharpe24'.padStart(9)} ${'Sharpe25'.padStart(9)} ${'Combined'.padStart(9)} ${'Trades'.padStart(7)} ${'PnL $'.padStart(10)} ${'AvgWR%'.padStart(7)} ${'MaxDD%'.padStart(7)}`;
  console.log(rHeader);
  console.log('-'.repeat(110));

  ranked.forEach((r, i) => {
    const line = `${String(i + 1).padEnd(5)} ${r.cfg.padEnd(4)} ${r.label.padEnd(38)} ${r.sharpe2024.toFixed(2).padStart(9)} ${r.sharpe2025.toFixed(2).padStart(9)} ${r.combinedSharpe.toFixed(2).padStart(9)} ${String(r.totalTrades).padStart(7)} ${('$' + r.totalPnl.toFixed(0)).padStart(10)} ${r.avgWR.toFixed(1).padStart(7)} ${r.maxDD.toFixed(1).padStart(7)}`;
    console.log(line);
  });
