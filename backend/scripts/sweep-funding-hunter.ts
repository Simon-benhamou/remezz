/**
 * Funding Hunter Parameter Sweep
 * Tests 12 configurations across 2024 and 2025 with BTC, ETH, SOL, XRP
 * Uses REAL historical funding rate data aligned with REAL price data.
 *
 * Usage: npx tsx scripts/sweep-funding-hunter.ts 2>&1
 */
...
// ============================================================================
const DEFAULTS = { ...FUNDING_HUNTER_CONFIG };
...
const CONFIGS: SweepConfig[] = [
  {
    name: 'A',
    label: 'Baseline >0.01% SL=3% 8h 5x',
    overrides: {},
  },
  {
    name: 'B',
    label: 'Higher thresh >0.03%',
    overrides: { HIGH_FUNDING_ENTRY: 0.03, LOW_FUNDING_ENTRY: -0.03 },
  },
  {
    name: 'C',
    label: 'Very high >0.05%',
    overrides: { HIGH_FUNDING_ENTRY: 0.05, LOW_FUNDING_ENTRY: -0.05 },
  },
  {
    name: 'D',
    label: 'Tighter SL=1.5%',
    overrides: { STOP_LOSS_PCT: 1.5 },
  },
  {
    name: 'E',
    label: 'Wider SL=5%',
    overrides: { STOP_LOSS_PCT: 5.0 },
  },
  {
    name: 'F',
    label: 'Longer hold 24h',
    overrides: { HOLD_CANDLES: 96, MAX_HOLD_CANDLES: 288 },
  },
  {
    name: 'G',
    label: 'Short-only',
  ...
  {
    name: 'H',
    label: 'No trailing, time exit',
    overrides: { TRAILING_ENABLED: false },
  },
  {
    name: 'I',
    label: 'Aggressive trail 0.5/0.3',
    overrides: { TRAILING_ACTIVATION_PCT: 0.5, TRAILING_DISTANCE_PCT: 0.3 },
  },
  {
    name: 'J',
    label: 'Conservative >0.03% 3x',
    overrides: { HIGH_FUNDING_ENTRY: 0.03, LOW_FUNDING_ENTRY: -0.03, STOP_LOSS_PCT: 3.0, HOLD_CANDLES: 64, MAX_HOLD_CANDLES: 192, TRAILING_ACTIVATION_PCT: 2.0, TRAILING_DISTANCE_PCT: 1.5, LEVERAGE: 3 },
  },
  {
    name: 'K',
    label: '10x lev SL=2%',
    overrides: { LEVERAGE: 10, STOP_LOSS_PCT: 2.0 },
  },
  {
    name: 'L',
    label: 'Best guess >0.02% SL=2% 16h',
    overrides: { HIGH_FUNDING_ENTRY: 0.02, LOW_FUNDING_ENTRY: -0.02, STOP_LOSS_PCT: 2.0, HOLD_CANDLES: 64, MAX_HOLD_CANDLES: 192, TRAILING_ACTIVATION_PCT: 1.5, TRAILING_DISTANCE_PCT: 1.0 },
  },
];
...
async function main() {
  ...
  console.log('FUNDING HUNTER PARAMETER SWEEP (REAL FUNDING DATA)');
  console.log('='.repeat(80));
  console.log(`Configs:  ${CONFIGS.length} (A-L)`);
  console.log(`Years:    ${YEARS.join(', ')}`);
  console.log(`Symbols:  ${SYMBOLS.map(s => s.replace('/USDT:USDT', '')).join(', ')}`);
  console.log(`Capital:  $${INITIAL_CAPITAL}`);
  console.log(`Default leverage: ${DEFAULTS.LEVERAGE}x`);
  console.log('');
  ...
  const header = `${'Cfg'.padEnd(4)} ${'Label'.padEnd(35)} ${'Year'.padEnd(5)} ${'Trades'.padStart(7)} ${'WR%'.padStart(6)} ${'PnL $'.padStart(10)} ${'Sharpe'.padStart(7)} ${'DD%'.padStart(6)} ${'PF'.padStart(6)} ${'AvgWin'.padStart(8)} ${'AvgLoss'.padStart(8)} ${'W/L'.padStart(5)}`;
  console.log(header);
  console.log('-'.repeat(130));

  for (const r of allResults) {
    const line = `${r.config.padEnd(4)} ${r.label.padEnd(35)} ${String(r.year).padEnd(5)} ${String(r.trades).padStart(7)} ${r.winRate.toFixed(1).padStart(6)} ${('$' + r.pnl.toFixed(0)).padStart(10)} ${r.sharpe.toFixed(2).padStart(7)} ${r.maxDD.toFixed(1).padStart(6)} ${r.profitFactor.toFixed(2).padStart(6)} ${('$' + r.avgWin.toFixed(1)).padStart(8)} ${('$' + r.avgLoss.toFixed(1)).padStart(8)} ${r.winLossRatio.toFixed(2).padStart(5)}`;
    console.log(line);
  }
  ...
  const rHeader = `${'Rank'.padEnd(5)} ${'Cfg'.padEnd(4)} ${'Label'.padEnd(35)} ${'Sharpe24'.padStart(9)} ${'Sharpe25'.padStart(9)} ${'Combined'.padStart(9)} ${'Trades'.padStart(7)} ${'PnL $'.padStart(10)} ${'AvgWR%'.padStart(7)} ${'MaxDD%'.padStart(7)}`;
  console.log(rHeader);
  console.log('-'.repeat(110));

  ranked.forEach((r, i) => {
    const line = `${String(i + 1).padEnd(5)} ${r.cfg.padEnd(4)} ${r.label.padEnd(35)} ${r.sharpe2024.toFixed(2).padStart(9)} ${r.sharpe2025.toFixed(2).padStart(9)} ${r.combinedSharpe.toFixed(2).padStart(9)} ${String(r.totalTrades).padStart(7)} ${('$' + r.totalPnl.toFixed(0)).padStart(10)} ${r.avgWR.toFixed(1).padStart(7)} ${r.maxDD.toFixed(1).padStart(7)}`;
    console.log(line);
  });
