...
class AgentPerformanceAnalyzer {
  ...
  async analyzeAllAgents(): Promise<PerformanceAnalysis> {
  ...
  private calculateStreaks(fills: any[]): { currentStreak: number; maxConsecutiveWins: number; maxConsecutiveLosses: number } {
    if (fills.length === 0) return { currentStreak: 0, maxConsecutiveWins: 0, maxConsecutiveLosses: 0 };
  ...
  private calculateAverageHoldingTime(fills: any[]): number {
    const entryExitPairs: { entry: any; exit: any }[] = [];
    const entries: any[] = [];
    ...
    if (entryExitPairs.length === 0) return 0;
  ...
  private calculateSharpeRatio(fills: any[]): number {
  ...
  private calculateCalmarRatio(totalPnL: number, maxDrawdown: number): number {
    return maxDrawdown > 0 ? totalPnL / maxDrawdown : 0;
  }

  private evaluateAgent(metrics: any): { issues: string[]; recommendations: string[]; score: number; grade: string } {
    const issues: string[] = [];
  ...
  private async generateAnalysis(agentMetrics: AgentMetrics[]): Promise<PerformanceAnalysis> {
    ...
    const bestPerformer = agentMetrics.reduce((best, current) =>
      current.score > best.score ? current : best, agentMetrics[0]);
    const worstPerformer = agentMetrics.reduce((worst, current) =>
      current.score < worst.score ? current : worst, agentMetrics[0]);
  ...
  private async analyzeMarketConditions(agentMetrics: AgentMetrics[]): Promise<any> {
  ...
  private generateAlertsFromMetrics(agentMetrics: AgentMetrics[]): any {
  ...
  private async logAnalysis(analysis: PerformanceAnalysis): Promise<void> {
    const logEntry = {
  ...
  private async saveAnalysisToDatabase(analysis: PerformanceAnalysis): Promise<void> {
    try {
  ...
  private async generateAlerts(analysis: PerformanceAnalysis): Promise<void> {
...
export { AgentPerformanceAnalyzer, AgentMetrics, PerformanceAnalysis };