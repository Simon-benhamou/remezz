/**
 * Mean Reversion 4h Parameter Sweep
 * Tests 10 configurations across 2024 and 2025 with BTC, ETH, SOL, XRP
 *
 * Usage: npx tsx scripts/sweep-mean-reversion-4h.ts 2>&1
 */
...
// ============================================================================
const DEFAULTS = { ...MEAN_REV_4H_CONFIG };
...
const CONFIGS: SweepConfig[] = [
  ...
  {
    name: 'C',
    label: 'Wide BB(20,2.5) SL=3%',
    overrides: { BB_STD_ENTRY: 2.5 },
  },
  {
    name: 'D',
    label: 'Narrow BB(20,1.5) SL=2%',
    overrides: { BB_STD_ENTRY: 1.5, STOP_LOSS_PCT: 2.0 },
  },
  {
    name: 'E',
    label: 'Let winners run Trail3/2.5',
    overrides: {
      TRAILING_ACTIVATION_PCT: 3.0,
      TRAILING_DISTANCE_PCT: 2.5,
      TIER2_PROFIT_PCT: 6.0,
      TIER2_TRAIL_PCT: 4.0,
      TIER3_PROFIT_PCT: 10.0,
      TIER3_TRAIL_PCT: 6.0,
    },
  },
  {
    name: 'F',
    label: 'Relaxed RSI 35/65',
    overrides: { RSI_OVERSOLD: 35, RSI_OVERBOUGHT: 65 },
  },
  {
    name: 'G',
    label: 'No ADX filter ADX_MAX=100',
    overrides: { ADX_MAX: 100 },
  },
  {
    name: 'H',
    label: 'Longer BB(30,2.0)',
    overrides: { BB_PERIOD: 30 },
  },
  {
    name: 'I',
    label: 'Combined RSI35/65 noADX SL=2%',
    overrides: { RSI_OVERSOLD: 35, RSI_OVERBOUGHT: 65, ADX_MAX: 100, STOP_LOSS_PCT: 2.0 },
  },
  {
    name: 'J',
    label: 'Very wide Trail4/3 Tier12/8',
    overrides: {
      TRAILING_ACTIVATION_PCT: 4.0,
      TRAILING_DISTANCE_PCT: 3.0,
      TIER2_PROFIT_PCT: 8.0,
      TIER2_TRAIL_PCT: 5.0,
      TIER3_PROFIT_PCT: 12.0,
      TIER3_TRAIL_PCT: 8.0,
    },
  },
];
...
async function main() {
  ...
  console.log('MEAN REVERSION 4H PARAMETER SWEEP');
  console.log('='.repeat(80));
  ...
  for (const year of YEARS) {
    ...
    // Need extra warmup for 4h aggregation: 640 * 15m = 160h of candles
    const extraBarsMs = 700 * 15 * 60 * 1000;
  ...
  for (const cfg of CONFIGS) {
    ...
    for (const year of YEARS) {
      ...
      const mutableCfg = MEAN_REV_4H_CONFIG as any;
      for (const [k, v] of Object.entries(cfg.overrides)) {
        mutableCfg[k] = v;
      }

      const strategy = new MeanReversion4hStrategy();
