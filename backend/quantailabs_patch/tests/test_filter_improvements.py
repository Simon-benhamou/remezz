...
class FilterImprovementsTest(unittest.TestCase):
    def test_adaptive_confidence_with_strong_adx(self):
        ...
        cfg = FilterConfig(confidence_threshold=0.50, adaptive_confidence=True)
        ...
        # Should pass because threshold reduced from 0.50 to 0.45
        self.assertTrue(ok)
    ...
    def test_adaptive_confidence_with_high_rr(self):
        ...
        facts = {
            "model_confidence": 0.46,  # Below base threshold
            "adx": 20.0,
            "atr": 0.5,
            "price": 100.0,
            "spread_bps": 5.0,
    ...
    def test_adaptive_confidence_with_multiple_strong_signals(self):
        ...
        facts = {
            "model_confidence": 0.44,  # Well below base threshold
            "adx": 26.0,  # Strong ADX (+0.05 reduction)
            "atr": 0.5,
            "price": 100.0,
            "spread_bps": 5.0,
            "dollar_volume": 300_000.0,  # High volume (+0.05 reduction)
            "rr_to_tp1": 2.5  # High RR (no additional reduction, max 2 signals)
        }
        
        ok, reasons = filters.evaluate_entry(facts)
        
        # Should pass because threshold reduced by 0.10 (max) to 0.40
        self.assertTrue(ok)

    def test_lower_adx_threshold_captures_more_opportunities(self):
        """Test that ADX threshold of 12.0 allows more setups"""
        cfg = FilterConfig(min_adx=12.0)
        filters = EntryFilters(cfg)
        
        facts = {
            "model_confidence": 0.60,
            "adx": 13.0,  # Would fail with old 15.0 threshold
            "atr": 0.5,
            "price": 100.0,
            "spread_bps": 5.0,
            "dollar_volume": 200_000.0,
    ...
    def test_lower_rr_threshold_captures_more_opportunities(self):
        """Test that RR threshold of 1.05 allows more setups"""
        cfg = FilterConfig(min_rr=1.05)
        filters = EntryFilters(cfg)
        ...
        facts = {
            ...
            "spread_bps": 5.0,
            "dollar_volume": 200_000.0,
            "rr_to_tp1": 1.1  # Would fail with old 1.3 threshold
        }
    ...
    def test_wider_spread_tolerance(self):
        """Test that max_spread_bps of 12.0 allows more flexibility"""
        cfg = FilterConfig(max_spread_bps=12.0)
        filters = EntryFilters(cfg)
        ...
        facts = {
            ...
            "atr": 0.5,
            "price": 100.0,
            "spread_bps": 11.0,  # Would fail with old 8.0 threshold
            "dollar_volume": 200_000.0,
            "rr_to_tp1": 1.5
        }
    ...
    def test_filters_still_reject_poor_quality_setups(self):
        ...
        facts = {
            ...
            "adx": 10.0,  # Weak momentum (below 12.0)
            "atr": 0.5,
            "price": 100.0,
            "spread_bps": 5.0,
            "dollar_volume": 100_000.0,  # Low volume (below 150k)
