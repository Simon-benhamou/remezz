#!/usr/bin/env node
/**
 * 📊 STRATÉGIE ADAPTATIVE V22 - SELECTIVE QUALITY ONLY
 * 
 * Approche: NE TRADER QUE LES SETUPS DE HAUTE QUALITÉ
 * 
 * Analyse des problèmes:
 * - Août et Sept: Marché choppy, beaucoup de faux signaux
 * - Oct-Nov: Tendance claire, profits faciles
 * 
 * Solution:
 * - Score de qualité composite pour chaque signal
 * - Ne prendre que les trades avec score >= 4/6
 * - Moins de trades = moins de pertes dans les mois difficiles
 */
...
function calculateIndicators(candles) {
  ...
  const change12h = (last - closes[closes.length - 49]) / closes[closes.length - 49] * 100;
  ...
  return {
    ...
    bbUpper: bb.upper,
    bbLower: bb.lower,
    change15m, change1h, change4h, change12h, trend72,
    bodyRatio,
 ...
 * 2. ADX >= 25
 * 3. Volume >= 1.5
 * 4. CMF fort (>0.08 / <-0.08)
 * 5. EMA Stack aligné
 * 6. RSI pas extrême (30-70)
 */
...
function makeAdaptiveDecision(ind) {
  ...
  // Minimum requirements pour considérer un trade
  const minVol = 1.2;
  ...
  if (
    ...
    // Besoin d'au moins 4/6 critères
    if (score >= 4) {
      decision = 'LONG';
      confidence = 0.58 + score * 0.03;
