#!/usr/bin/env node
/**
 * 📊 STRATÉGIE ADAPTATIVE V24 - ULTRA SELECTIVE
 * 
 * Problème identifié: Trop de trades en Août-Sept = accumulation de pertes
 * 
 * Solution: 
 * - Maximum 1 trade par jour par asset
 * - Cooldown de 24h après chaque trade
 * - Exigences plus strictes pour les signaux
 */
...
const CONFIG = {
  ...
  cooldownBars: 96, // 24h cooldown (96 * 15min)
  minScore: 5,
};
...
function calculateQualityScore(ind, side) {
  ...
  // 6. RSI zone idéale (40-60 au lieu de 30-70)
...
function makeAdaptiveDecision(ind) {
  ...
  const minVol = 1.5; // Plus strict
  const minBodyRatio = 0.5; // Plus strict
  
  if (volumeRatio < minVol || bodyRatio < minBodyRatio) {
    return { decision: 'NO_TRADE', confidence: 0, reasons: ['WEAK_SIGNAL'], score: 0 };
  }
  
  // Check LONG setup - criteria plus stricts
  if (
    change1h > 0.3 && // Plus strict
    change4h > 0.5 && // Plus strict
    cmf20 > 0.05 && // Plus strict
    trend > 0 &&
    isBullishCandle
  ) {
  ...
  // Check SHORT setup - criteria plus stricts
  if (
    change1h < -0.3 && // Plus strict
    change4h < -0.5 && // Plus strict
    cmf20 < -0.05 && // Plus strict
...
async function backtestSymbol(symbol, candles) {
  ...
  const lookback = 100;
  let lastTradeBar = -CONFIG.cooldownBars; // Allow first trade immediately
  
  for (let i = lookback; i < candles.length - 96; i++) {
    // STRICT COOLDOWN: 24h between trades on same asset
...
async function main() {
  ...
  console.log('📊 STRATÉGIE ADAPTATIVE V24 - ULTRA SELECTIVE');
  console.log('═'.repeat(80));
