#!/usr/bin/env node
/**
 * 📊 STRATÉGIE ADAPTATIVE V28 - TIGHT STOPS
 * 
 * V24 = 3/4 mois positifs. Septembre perd -2.13%
 * 
 * Nouvelle approche: Réduire les pertes avec des stops plus serrés
 * - Stop = 1.0x ATR au lieu de 1.5x
 * - Mais garder les TP identiques
 * - Meilleur Risk/Reward
 * 
 * Même si on perd plus souvent, les pertes seront plus petites
 */
...
const CONFIG = {
  ...
  // NOUVEAU: Stops plus serrés
  stopMultMed: 1.0, // Était 1.3
  stopMultHigh: 1.2, // Était 1.5
};
...
function simulateTrade(ind, futureCandles, side) {
  ...
  if (volatilityRegime === 'MEDIUM') {
    stopMult = CONFIG.stopMultMed; // NOUVEAU: Plus serré
    tp1Mult = 1.8; 
    tp2Mult = 3.0; 
    maxHold = 64;
  } else {
    stopMult = CONFIG.stopMultHigh; // NOUVEAU: Plus serré
    tp1Mult = 2.2; 
    tp2Mult = 4.0; 
    maxHold = 96;
  }
...
async function backtestSymbol(symbol, candles) {
  ...
  for (let i = lookback; i < candles.length - 96; i++) {
    ...
    const { decision, score, reasons, confidence } = makeAdaptiveDecision(indicators);
...
async function main() {
  ...
  console.log('📊 STRATÉGIE ADAPTATIVE V28 - TIGHT STOPS');
  console.log('═'.repeat(80));
  ...
  console.log(`🛑 Stop Med: ${CONFIG.stopMultMed}x ATR | Stop High: ${CONFIG.stopMultHigh}x ATR`);
  console.log('═'.repeat(80));
