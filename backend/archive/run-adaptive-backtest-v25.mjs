#!/usr/bin/env node
/**
 * 📊 STRATÉGIE ADAPTATIVE V25 - CMF STRICT
 * 
 * Basé sur V24 avec ajustements basés sur l'analyse de Septembre:
 * - CMF |cmf| >= 0.15 (perdants avaient 0.20, gagnants 0.25)
 * - Volume >= 2.5 (perdants avaient 3.8, gagnants 4.4)
 * 
 * Ces ajustements devraient filtrer les trades faibles de septembre
 */
...
const CONFIG = {
  ...
  // Ultra selective mode - ajusté pour septembre
  cooldownBars: 96,
  minScore: 5,
  
  // Filtres additionnels basés sur analyse septembre
  minCMF: 0.15, // Plus strict (perdants avaient 0.20 moyenne)
  minVolume: 2.5, // Plus strict (perdants avaient 3.8 moyenne)
};
...
function makeAdaptiveDecision(ind) {
  ...
  // Ultra strict requirements - basé sur bodyRatio
  if (bodyRatio < 0.5) {
    return { decision: 'NO_TRADE', confidence: 0, reasons: ['WEAK_BODY'], score: 0 };
  }
  
  // NOUVEAU: CMF minimum strict (basé sur analyse septembre)
  if (Math.abs(cmf20) < CONFIG.minCMF) {
    return { decision: 'NO_TRADE', confidence: 0, reasons: ['WEAK_CMF'], score: 0 };
  }
  
  // NOUVEAU: Volume minimum strict (basé sur analyse septembre)
  if (volumeRatio < CONFIG.minVolume) {
    return { decision: 'NO_TRADE', confidence: 0, reasons: ['WEAK_VOL'], score: 0 };
  }
  ...
  if (
    ...
    if (score >= CONFIG.minScore) {
      return { decision: 'LONG', confidence: 0.62 + score * 0.03, reasons, score };
    }
  }
  ...
  if (
    ...
    if (score >= CONFIG.minScore) {
      return { decision: 'SHORT', confidence: 0.64 + score * 0.03, reasons, score };
    }
  }
...
async function main() {
  ...
  console.log('📊 STRATÉGIE ADAPTATIVE V25 - CMF STRICT');
  console.log('═'.repeat(80));
  ...
  console.log(`⏱️ Cooldown: ${CONFIG.cooldownBars} bars (${CONFIG.cooldownBars * 15 / 60}h)`);
  console.log(`📊 Min CMF: ${CONFIG.minCMF}`);
  console.log(`📊 Min Volume: ${CONFIG.minVolume}x`);
  console.log('═'.repeat(80));
