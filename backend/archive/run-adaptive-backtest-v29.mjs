#!/usr/bin/env node
/**
 * 📊 STRATÉGIE ADAPTATIVE V29 - ADAPTIVE POSITION SIZE
 * 
 * V24 était 3/4 mois positifs avec +6.45%
 * 
 * Nouvelle idée: Réduire la taille de position quand on perd
 * - Après 2 pertes consécutives, réduire le risk à 0.5%
 * - Après 1 gain, revenir à 1%
 * 
 * Cela devrait limiter les dégâts en septembre tout en gardant
 * les gains des bons mois
 */
...
const CONFIG = {
  ...
  days: 120,
  equityUsd: 10000,
  riskPerTradeNormal: 0.01,    // 1% normal
  riskPerTradeReduced: 0.005,  // 0.5% après 2 pertes
  ...
  // Règle de réduction
  consecutiveLossesForReduction: 2,
};
...
function simulateTrade(ind, futureCandles, side) {
  ...
  return { side, pnlPct, exitReason, holdBars, volatilityRegime, stopMult: volatilityRegime === 'MEDIUM' ? 1.3 : 1.5 };
}

async function main() {
  console.log('═'.repeat(80));
  console.log('📊 STRATÉGIE ADAPTATIVE V29 - ADAPTIVE POSITION SIZE');
  console.log('═'.repeat(80));
  ...
  console.log(`📉 Position après ${CONFIG.consecutiveLossesForReduction} pertes: ${CONFIG.riskPerTradeReduced * 100}%`);
  console.log('═'.repeat(80));
  ...
  // Combined backtest with adaptive position sizing
  const allTrades = [];
  let equity = CONFIG.equityUsd;
  let consecutiveLosses = 0;
  let reducedRiskTrades = 0;
  ...
  // Process trades in chronological order with adaptive sizing
  for (const trade of potentialTrades) {
    ...
    // Determine risk based on consecutive losses
    const currentRisk = consecutiveLosses >= CONFIG.consecutiveLossesForReduction 
      ? CONFIG.riskPerTradeReduced 
      : CONFIG.riskPerTradeNormal;
    
    if (currentRisk === CONFIG.riskPerTradeReduced) {
      reducedRiskTrades++;
    }
    
    // Calculate PnL
    const riskAmount = equity * currentRisk;
    ...
    allTrades.push({
      ...
      score,
      reasons,
      reducedRisk: currentRisk === CONFIG.riskPerTradeReduced,
    });
...
├───────────┼────────┼────────┼───────────┼────────────────┼──────────────┤`);
  ...
  console.log(`   Trades à risque réduit: ${reducedRiskTrades} (${(reducedRiskTrades / totalTrades * 100).toFixed(1)}%)`);
