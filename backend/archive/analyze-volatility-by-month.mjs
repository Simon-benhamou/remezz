#!/usr/bin/env node
/**
 * 📊 ANALYSE DE VOLATILITÉ PAR MOIS + BACKTEST HIGH ONLY
 * 
 * Questions à répondre:
 * 1. Combien de signaux HIGH volatility par mois?
 * 2. Quelle performance si on trade UNIQUEMENT en HIGH?
 * 3. Est-ce que HIGH est présent chaque mois?
 */
...
function makeAdaptiveDecision(ind) {
  // Filtrer par régime de volatilité
  if (!CONFIG.allowedRegimes.includes(ind.volatilityRegime)) {
    return { decision: 'NO_TRADE', score: 0, reason: 'NOT_HIGH_VOL' };
  }
  
  if (ind.volumeRatio < 1.5 || ind.bodyRatio < 0.5) {
    return { decision: 'NO_TRADE', score: 0, reason: 'FILTER' };
  }
...
function simulateTrade(ind, futureCandles, side) {
  ...
  // HIGH volatility parameters (comme dans les tests 2 mois qui marchaient bien)
  const stopMult = 1.5;
  const tp1Mult = 2.2;
  const tp2Mult = 4.0;
  const maxHold = 96;
...
async function main() {
  ...
  console.log('📊 ANALYSE VOLATILITÉ PAR MOIS + BACKTEST HIGH ONLY');
  console.log('═'.repeat(80));
  ...
  // Analyser la distribution de volatilité par mois
  const volatilityByMonth = {};
  const lookback = 100;
  
  console.log('\n' + '═'.repeat(80));
  console.log('📈 DISTRIBUTION DE VOLATILITÉ PAR MOIS');
  console.log('═'.repeat(80));
  ...
  for (const symbol of CONFIG.symbols) {
    ...
    for (let i = lookback; i < candles.length; i++) {
      ...
      if (!volatilityByMonth[monthKey]) {
        volatilityByMonth[monthKey] = { HIGH: 0, MEDIUM: 0, LOW: 0, total: 0 };
      }
      
      volatilityByMonth[monthKey][indicators.volatilityRegime]++;
      volatilityByMonth[monthKey].total++;
    }
  }
  
  console.log(`
┌───────────┬────────────┬────────────┬────────────┬──────────────────────────────┐
│   Mois    │    HIGH    │   MEDIUM   │    LOW     │         Distribution         │
├───────────┼────────────┼────────────┼────────────┼──────────────────────────────┤`);
  
  for (const month of Object.keys(volatilityByMonth).sort()) {
    const data = volatilityByMonth[month];
    const highPct = (data.HIGH / data.total * 100).toFixed(1);
    const medPct = (data.MEDIUM / data.total * 100).toFixed(1);
    const lowPct = (data.LOW / data.total * 100).toFixed(1);
    
    // Visual bar
    const highBars = Math.round(data.HIGH / data.total * 20);
    const medBars = Math.round(data.MEDIUM / data.total * 20);
    const lowBars = 20 - highBars - medBars;
    const bar = '🔴'.repeat(highBars) + '🟡'.repeat(medBars) + '🟢'.repeat(lowBars);
    
    console.log(`│ ${month} │ ${highPct.padStart(8)}%  │ ${medPct.padStart(8)}%  │ ${lowPct.padStart(8)}%  │ ${bar.substring(0, 30).padEnd(28)} │`);
  }
  console.log(`└───────────┴────────────┴────────────┴────────────┴──────────────────────────────┘`);
  console.log(`   🔴 = HIGH volatility | 🟡 = MEDIUM | 🟢 = LOW`);
  
  // Backtest HIGH only
  console.log('\n' + '═'.repeat(80));
  console.log('📊 BACKTEST HIGH VOLATILITY ONLY');
  console.log('═'.repeat(80));
  ...
  for (const trade of allTrades) {
    ...
    if (!monthlyResults[monthKey]) {
      monthlyResults[monthKey] = { trades: [], wins: 0, losses: 0, totalPnlUsd: 0 };
    }
...
├───────────┼────────┼────────┼───────────┼────────────────┼──────────────┤`);
  ...
  console.log('\n' + '═'.repeat(80));
  console.log('📊 RÉSUMÉ FINAL - HIGH VOLATILITY ONLY');
  console.log('═'.repeat(80));
