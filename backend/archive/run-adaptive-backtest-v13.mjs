#!/usr/bin/env node
/**
 * 📊 STRATÉGIE ADAPTATIVE V13 - TREND FILTER SIMPLE
 * 
 * Retour aux bases: V8 + filtre de tendance simple
 * 
 * Le problème d'Août-Sept: Le marché était en range/choppy
 * → Solution: Ne trader que quand il y a une VRAIE tendance
 * 
 * FILTRE SIMPLE:
 * - Calculer le momentum sur 3 jours (72 candles)
 * - Si mouvement net < 2% → SKIP (pas de tendance)
 * - Si mouvement net > 2% → TRADE dans la direction
 */
...
const CONFIG = {
  ...
  // Trend Filter
  trendFilter: {
    lookbackBars: 72,       // 3 jours (72 candles de 15min = 18h de trading)
    minTrendPct: 1.5,       // Minimum 1.5% de mouvement net
    updateEvery: 16,        // Recalculer toutes les 4h
  },
};
...
/**
 * 📈 TREND FILTER - Simple et efficace
 */
...
function makeAdaptiveDecision(ind, trendInfo) {
  ...
  // ═══════════════════════════════════════════════════════════════
  // TREND FILTER - Skip si pas de tendance
  // ═══════════════════════════════════════════════════════════════
  ...
  // Bonus si trade dans la direction du trend
  ...
  // ═══════════════════════════════════════════════════════════════
  // MEDIUM VOLATILITY - Très sélectif
  // ═══════════════════════════════════════════════════════════════
  if (volatilityRegime === 'MEDIUM') {
    strategy = 'MED';
    riskMultiplier = 0.5;
    
    // Seulement dans la direction du trend
...
async function backtestSymbol(symbol, candles) {
  ...
  return { symbol, trades, equity, maxDrawdown, byVolRegime, noTrendCount };
}

// Main
async function main() {
  console.log('═'.repeat(80));
  console.log('📊 STRATÉGIE ADAPTATIVE V13 - TREND FILTER');
  console.log('═'.repeat(80));
  console.log(`📅 Période: ${CONFIG.days} jours (${Math.round(CONFIG.days/30)} mois)`);
  console.log(`💰 Capital: $${CONFIG.equityUsd.toLocaleString()}`);
  console.log(`📈 Trend Filter: Min ${CONFIG.trendFilter.minTrendPct}% move sur ${CONFIG.trendFilter.lookbackBars} bars`);
  console.log('═'.repeat(80));
