#!/usr/bin/env node
/**
 * 📊 STRATÉGIE ADAPTATIVE V26 - NO EARLY MONTH TRADING
 * 
 * Observation: Les pertes semblent concentrées en début de mois (1-7)
 * 
 * Solution: V24 (qui était 3/4) + ne pas trader les 7 premiers jours du mois
 * 
 * Cette approche simple peut éliminer beaucoup de faux signaux
 */
...
const CONFIG = {
  ...
  // Nouveau: Skip les 5 premiers jours du mois
  skipDaysStart: 5,
};
...
async function backtestSymbol(symbol, candles) {
  ...
  for (let i = lookback; i < candles.length - 96; i++) {
    ...
    // NOUVEAU: Skip les X premiers jours du mois
    if (date.getDate() <= CONFIG.skipDaysStart) continue;
...
async function main() {
  ...
  console.log('📊 STRATÉGIE ADAPTATIVE V26 - NO EARLY MONTH TRADING');
  console.log('═'.repeat(80));
  ...
  console.log(`🚫 Skip: Jours 1-${CONFIG.skipDaysStart} de chaque mois`);
  console.log('═'.repeat(80));
