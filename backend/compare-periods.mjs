#!/usr/bin/env node
/**
 * 📊 COMPARAISON 60 JOURS VS 120 JOURS
 * 
 * Les tests 2 mois marchaient bien, les 4 mois moins.
 * Analysons les différences par période de 2 mois.
 */
...
async function main() {
  ...
  console.log('📊 COMPARAISON PAR PÉRIODE DE 60 JOURS');
  console.log('═'.repeat(80));
  ...
  // Découper en 2 périodes de 60 jours
  const periods = [
    { name: 'Période 1 (Août-Sept)', startDay: 0, endDay: 60 },
    { name: 'Période 2 (Oct-Nov)', startDay: 60, endDay: 120 },
  ];
  ...
  for (const period of periods) {
    ...
    const lookback = 100;
    const barsPerDay = 96;
    const startBar = lookback + period.startDay * barsPerDay;
    const endBar = lookback + period.endDay * barsPerDay;
    ...
    for (const symbol of CONFIG.symbols) {
      ...
      for (let i = startBar; i < Math.min(endBar, candles.length - 96); i++) {
    ...
    for (const trade of potentialTrades) {
      ...
      if (result.volatilityRegime === 'HIGH') highTrades++;
      else medTrades++;
    ...
    for (const trade of allTrades) {
      ...
      if (!monthlyResults[monthKey]) {
        monthlyResults[monthKey] = { wins: 0, losses: 0, totalPnlUsd: 0 };
      }
    ...
    for (const month of months) {
      ...
      const monthlyReturn = (data.totalPnlUsd / cumCap) * 100;
      cumCap += data.totalPnlUsd;
      const isPositive = monthlyReturn >= 0;
      if (isPositive) positiveMonths++;
      const icon = isPositive ? '✅' : '❌';
      console.log(`   ${month}: ${String(totalTrades).padStart(2)} trades | ${winRate.toFixed(1).padStart(5)}% WR | ${icon} ${monthlyReturn >= 0 ? '+' : ''}${monthlyReturn.toFixed(2)}%`);
    }
    ...
    console.log(`\n   📊 Total: ${totalTrades} trades | ${overallWinRate.toFixed(1)}% WR | ${cumulativeReturn >= 0 ? '+' : ''}${cumulativeReturn.toFixed(2)}%`);
    console.log(`   🔴 HIGH: ${highTrades} | 🟡 MEDIUM: ${medTrades}`);
  ...
  console.log('═'.repeat(80));
  console.log(`
   La période Août-Septembre a moins de HIGH volatility et plus de LOW
   → Moins d'opportunités de qualité
   → Les trades MEDIUM en basse volatilité sous-performent
  `);
}
