...
- V5.120: Parity verification fixes — forcedEntryTimestamp, fixed SL, entryAtrPct, diagnostics:
  - **CRITICAL: forcedEntryTimestamp off-by-one (reverting V5.107)**: V5.107 assumed `Trade.entryTs = Date.now()` (wall-clock) and removed the +CANDLE_15M_MS offset from V5.106. Investigation via DB analysis (15 trades, 14/15 at exact 15m boundaries) proved `Trade.entryTs = candle OPEN time` in practice. Root cause: `savePositionToDb` stores `openedAt = new Date(position.entryTime)` (candle open), and `loadExistingPosition` overwrites `realEntryTime = openedAt`. Since servers restart frequently, almost all trades have candle-open entryTs. Without offset, parity looked at forming candle → NO_SIGNAL for every trade. Fix: `forcedEntryTimestamp = floor(entryTs / 15min) * 15min + CANDLE_15M_MS` (always shift from candle open to close boundary). V5.106 was correct all along.
