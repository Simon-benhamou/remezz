#!/usr/bin/env node
/**
 * 📊 STRATÉGIE ADAPTATIVE V12 - PERFORMANCE FEEDBACK LOOP
 * 
 * OBJECTIF: Chaque mois doit être positif
 * 
 * APPROCHE PRAGMATIQUE:
 * Utiliser la stratégie V8 qui marche (+7.26% sur 4 mois)
 * MAIS ajouter un "Performance Feedback Loop":
 * 
 * - Track les X derniers trades
 * - Si WR < 40% sur les derniers trades → PAUSE trading
 * - Si on perd > 3% du capital → PAUSE trading 24h
 * - Reprendre quand conditions s'améliorent
 * 
 * C'est simple mais efficace: arrêter de saigner quand ça va mal.
 */
...
const CONFIG = {
  ...
  feedback: {
    lookbackTrades: 5,        // Regarder les 5 derniers trades
    minWinRate: 0.35,         // Pause si WR < 35%
    maxDrawdownPause: 0.025,  // Pause si -2.5% drawdown récent
    pauseBars: 96,            // Pause pendant 24h (96 candles de 15min)
    recoveryWins: 2,          // Besoin de 2 wins consécutifs pour reprendre
  },
};
...
/**
 * 🎯 STRATÉGIE V8 (COPIE EXACTE) + Feedback State Check
 */
function makeAdaptiveDecision(ind, feedbackState) {
  const { 
  ...
  // ═══════════════════════════════════════════════════════════════
  // PERFORMANCE FEEDBACK FILTER
  // ═══════════════════════════════════════════════════════════════
  if (feedbackState.isPaused) {
    return { 
      decision: 'NO_TRADE', 
      confidence: 0, 
      reasons: [`PAUSED_${feedbackState.pauseReason}`], 
      strategy: 'FEEDBACK_PAUSE',
      riskMultiplier: 0,
    };
  }
  
  // Réduire le risque si performance récente est mauvaise
  if (feedbackState.recentWR < 0.45) {
    riskMultiplier *= 0.7;
  }
  ...
  if (volatilityRegime === 'LOW') {
    return { decision: 'NO_TRADE', confidence: 0, reasons: ['SKIP_LOW'], strategy: 'LOW', riskMultiplier: 0 };
  }
  
  // ═══════════════════════════════════════════════════════════════
  // MEDIUM VOLATILITY - Ultra strict
  // ═══════════════════════════════════════════════════════════════
  if (volatilityRegime === 'MEDIUM') {
    ...
    if (adx14 >= 28 && volumeRatio >= 1.8 && bodyRatio >= 0.6) {
      if (
        ...
        decision = 'SHORT';
        confidence = 0.67;
        reasons.push('MED_DOWN');
      }
    }
  }
  
  // ═══════════════════════════════════════════════════════════════
  // HIGH VOLATILITY - V8 strategies
  // ═══════════════════════════════════════════════════════════════
  ...
  if (decision !== 'NO_TRADE' && volumeRatio >= 3.0) confidence += 0.05;
  
  const minConfidence = volatilityRegime === 'MEDIUM' ? 0.58 : 0.58;
 ...
 */
function updateFeedbackState(state, trade, currentBar) {
  const { lookbackTrades, minWinRate, maxDrawdownPause, pauseBars, recoveryWins } = CONFIG.feedback;
  
  // Add trade to history
  state.recentTrades.push({ pnlPct: trade.pnlPct, bar: currentBar });
  
  // Keep only lookback trades
  if (state.recentTrades.length > lookbackTrades) {
    state.recentTrades.shift();
  }
  
  // Update drawdown
  if (trade.pnlPct < 0) {
    state.recentDrawdown += Math.abs(trade.pnlPct) / 100;
    state.consecutiveWins = 0;
  } else {
    state.recentDrawdown = Math.max(0, state.recentDrawdown - trade.pnlPct / 200); // Slow recovery
    state.consecutiveWins++;
  }
  
  // Calculate recent win rate
  const wins = state.recentTrades.filter(t => t.pnlPct > 0).length;
  state.recentWR = state.recentTrades.length > 0 ? wins / state.recentTrades.length : 0.5;
  
  // Check if should pause
  if (state.recentTrades.length >= lookbackTrades && state.recentWR < minWinRate) {
    state.isPaused = true;
    state.pauseUntil = currentBar + pauseBars;
    state.pauseReason = `WR_${(state.recentWR * 100).toFixed(0)}`;
  }
  
  if (state.recentDrawdown > maxDrawdownPause) {
    state.isPaused = true;
    state.pauseUntil = currentBar + pauseBars;
    state.pauseReason = `DD_${(state.recentDrawdown * 100).toFixed(1)}`;
  }
  
  return state;
}

function checkPauseStatus(state, currentBar) {
  // Check if pause period is over
  if (state.isPaused && currentBar >= state.pauseUntil) {
    // Need recovery wins to resume
    if (state.consecutiveWins >= CONFIG.feedback.recoveryWins || state.recentWR >= 0.45) {
      state.isPaused = false;
      state.recentDrawdown = 0;
      state.pauseReason = '';
    }
  }
...
async function backtestSymbol(symbol, candles) {
  ...
  for (let i = lookback; i < candles.length - 96; i++) {
    // Check pause status
    feedbackState = checkPauseStatus(feedbackState, i);
    ...
    const { decision, confidence, reasons, strategy, riskMultiplier } = makeAdaptiveDecision(indicators, feedbackState);
    
    if (decision === 'NO_TRADE') {
      if (reasons[0]?.includes('PAUSED')) pausedCount++;
      continue;
    }
    ...
    feedbackState = updateFeedbackState(feedbackState, result, i);
  ...
  return { symbol, trades, equity, maxDrawdown, byVolRegime, pausedCount };
}

// Main
async function main() {
  console.log('═'.repeat(80));
  console.log('📊 STRATÉGIE ADAPTATIVE V12 - PERFORMANCE FEEDBACK LOOP');
  console.log('═'.repeat(80));
  console.log(`📅 Période: ${CONFIG.days} jours (${Math.round(CONFIG.days/30)} mois)`);
  console.log(`💰 Capital: $${CONFIG.equityUsd.toLocaleString()}`);
  console.log(`🔄 Feedback: Pause si WR<${CONFIG.feedback.minWinRate*100}% ou DD>${CONFIG.feedback.maxDrawdownPause*100}%`);
  console.log('═'.repeat(80));
  ...
  for (const symbol of CONFIG.symbols) {
    ...
    totalPaused += result.pausedCount;
    ...
    console.log(`   🔄 Paused: ${result.pausedCount} opportunities`);
  }
...
├───────────┼────────┼────────┼───────────┼────────────────┼──────────────┤`);
  ...
  // Regime breakdown
  console.log('\n' + '═'.repeat(80));
  console.log('📊 PERFORMANCE PAR RÉGIME DE VOLATILITÉ');
  console.log('═'.repeat(80));
  ...
  console.log(`   ${'PAUSED'.padEnd(8)}: ${totalPaused} opportunities skipped by feedback`);
