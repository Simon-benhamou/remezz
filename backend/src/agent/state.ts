  ...
  */
  ...
  private confirmEntrySignal(
    ...
    if (!volumeConfirmed) {
      return {
        confirmed: false,
        reason: `Waiting for volume confirmation (current: ${volumeRatio.toFixed(2)}x, need 1.2x)`
      };
    }
    ...
    return {
      confirmed: true,
      reason: `Entry confirmed: ${timeInZoneMin.toFixed(1)}min in zone, momentum reversed, volume ${volumeRatio.toFixed(2)}x`
    };
  }
