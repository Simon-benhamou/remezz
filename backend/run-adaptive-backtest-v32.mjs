#!/usr/bin/env node
/**
 * 📊 STRATÉGIE ADAPTATIVE V32 - SCORE DIFFÉRENT PAR RÉGIME
 * 
 * Insight de V31:
 * - HIGH: 50% WR, avg +0.303% → Score 5/6 OK
 * - MEDIUM: 52.3% WR, avg +0.048% → Besoin de filtrage plus strict
 * 
 * Nouvelle règle:
 * - HIGH volatility: Score minimum 5/6 (comme avant)
 * - MEDIUM volatility: Score minimum 6/6 (uniquement les meilleurs)
 */
...
const CONFIG = {
  ...
  allowedRegimes: ['HIGH', 'MEDIUM'],
  
  // Score différent par régime
  minScoreHigh: 5,
  minScoreMedium: 6,  // Plus strict pour MEDIUM
...
function makeAdaptiveDecision(ind) {
  if (!CONFIG.allowedRegimes.includes(ind.volatilityRegime)) {
    return { decision: 'NO_TRADE', score: 0 };
  }
  
  if (ind.volumeRatio < 1.5 || ind.bodyRatio < 0.5) {
    return { decision: 'NO_TRADE', score: 0 };
  }
  
  // Score minimum différent selon le régime
  const minScore = ind.volatilityRegime === 'HIGH' ? CONFIG.minScoreHigh : CONFIG.minScoreMedium;
  ...
  if (ind.change1h > 0.3 && ind.change4h > 0.5 && ind.cmf20 > 0.05 && ind.trend > 0 && ind.isBullishCandle) {
    ...
    if (score >= minScore) return { decision: 'LONG', score, reasons };
  }
  ...
  if (ind.change1h < -0.3 && ind.change4h < -0.5 && ind.cmf20 < -0.05 && ind.trend < 0 && ind.isBearishCandle) {
    ...
    if (score >= minScore) return { decision: 'SHORT', score, reasons };
  }
...
function simulateTrade(ind, futureCandles, side) {
  ...
  for (let i = 0; i < futureCandles.length && i < maxHold; i++) {
    ...
    if (hitTp1) {
      const trailMult = volatilityRegime === 'HIGH' ? 0.5 : 0.4;
  ...
  return { side, pnlPct, exitReason, holdBars, volatilityRegime, stopMult };
}

async function main() {
  console.log('═'.repeat(80));
  console.log('📊 STRATÉGIE ADAPTATIVE V32 - SCORE DIFFÉRENT PAR RÉGIME');
  console.log('═'.repeat(80));
  console.log(`📅 Période: ${CONFIG.days} jours (${Math.round(CONFIG.days/30)} mois)`);
  console.log(`💰 Capital: $${CONFIG.equityUsd.toLocaleString()}`);
  console.log(`🎯 Score minimum HIGH: ${CONFIG.minScoreHigh}/6`);
  console.log(`🎯 Score minimum MEDIUM: ${CONFIG.minScoreMedium}/6 (plus strict)`);
  ...
  const regimeStats = { HIGH: { wins: 0, losses: 0, pnl: 0 }, MEDIUM: { wins: 0, losses: 0, pnl: 0 } };
  ...
  for (const trade of potentialTrades) {
    ...
    const stopDistance = indicators.atrPct * result.stopMult;
    ...
    if (result.pnlPct > 0) regimeStats[regime].wins++;
    else regimeStats[regime].losses++;
    regimeStats[regime].pnl += result.pnlPct;
  ...
  for (const trade of allTrades) {
    ...
    if (!monthlyResults[monthKey]) {
      monthlyResults[monthKey] = { trades: [], wins: 0, losses: 0, totalPnlUsd: 0, highTrades: 0, medTrades: 0 };
    }
    ...
    if (trade.volatilityRegime === 'HIGH') monthlyResults[monthKey].highTrades++;
    else monthlyResults[monthKey].medTrades++;
  }
  ...
  console.log(`
┌───────────┬────────┬────────┬───────────┬────────────────┬───────────────────┐
│   Mois    │ Trades │   WR   │  Return   │     PnL ($)    │   HIGH / MEDIUM   │
├───────────┼────────┼────────┼───────────┼────────────────┼───────────────────┤`);
  ...
  for (const month of months) {
    ...
    if (isPositive) positiveMonths++;
    const icon = isPositive ? '✅' : '❌';
    console.log(`│ ${month} │ ${String(totalTrades).padStart(6)} │ ${winRate.toFixed(1).padStart(5)}% │ ${icon} ${monthlyReturn >= 0 ? '+' : ''}${monthlyReturn.toFixed(2).padStart(5)}% │ ${data.totalPnlUsd >= 0 ? '+' : ''}$${data.totalPnlUsd.toFixed(0).padStart(12)} │  🔴 ${String(data.highTrades).padStart(2)} / 🟡 ${String(data.medTrades).padStart(2)}   │`);
  }
  ...
  for (const [regime, stats] of Object.entries(regimeStats)) {
    ...
    const avgPnl = (stats.pnl / total).toFixed(3);
    const icon = regime === 'HIGH' ? '🔴' : '🟡';
    console.log(`   ${icon} ${regime}: ${String(total).padStart(3)} trades | ${String(wr).padStart(5)}% WR | ${stats.pnl >= 0 ? '+' : ''}${stats.pnl.toFixed(2).padStart(7)}% | Avg: ${avgPnl}%`);
  }
