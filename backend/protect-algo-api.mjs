// Place stops via Binance Algo Order API endpoint
...
console.log('\n=== PLACEMENT DES STOPS via ALGO ORDER API ===\n');
...
for (const p of openPos) {
  ...
  const existingAlgo = await binanceRequest('GET', '/fapi/v1/algo/openOrders', { symbol });
  if (existingAlgo.orders && existingAlgo.orders.length > 0) {
    console.log(`  ⏭️  Deja ${existingAlgo.orders.length} algo ordres - skip`);
    continue;
  }
  
  // Place via ALGO Order API endpoint
  console.log(`  📤 Placement via /fapi/v1/algo/order...`);
  
  // For STOP_MARKET via Algo API
  const orderParams = {
    symbol: symbol,
    side: closeSide,
    type: 'STOP_MARKET',
  ...
  const result = await binanceRequest('POST', '/fapi/v1/algo/order', orderParams);
